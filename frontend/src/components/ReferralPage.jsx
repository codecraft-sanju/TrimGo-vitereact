...
const ReferralPage = ({ user, onBack }) => {
  ...
  return (
    // UPDATED: min-h-[100dvh] for mobile browsers, overscroll-none to stop browser bounce
    <div className="min-h-[100dvh] w-full bg-[#F2F4F8] font-sans text-zinc-900 selection:bg-indigo-500/30 relative overscroll-y-none">
      
      {/* Background Decor - Hardware Accelerated for Mobile */}
      {/* UPDATED: Added overflow-hidden HERE strictly to contain the blobs */}
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <div className="absolute top-[-10%] right-[-5%] w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-purple-200/40 rounded-full blur-[80px] md:blur-[100px] transform-gpu" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[400px] md:w-[600px] h-[400px] md:h-[600px] bg-indigo-200/40 rounded-full blur-[80px] md:blur-[100px] transform-gpu" />
        ...
        <div className="max-w-5xl mx-auto pointer-events-auto">
          <div className="flex items-center justify-between bg-white/70 backdrop-blur-xl rounded-full px-2 py-2 pr-6 border border-white/50 shadow-sm transition-all duration-300">
            ...
            <div className="flex flex-col items-end">
              <span className="text-[10px] md:text-xs font-bold text-zinc-400 uppercase tracking-widest">Partner Hub</span>
      ...
      {/* --- MAIN CONTENT --- */}
      {/* UPDATED: Reduced top padding on mobile (pt-24) vs desktop (pt-32) */}
      <main className="relative z-10 max-w-5xl mx-auto px-4 pb-24 pt-24 md:pt-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6">
          ...
          >
            <div className="relative overflow-hidden rounded-[2rem] md:rounded-[2.5rem] bg-zinc-900 p-6 md:p-10 text-white shadow-2xl shadow-zinc-900/20 border border-zinc-800 transition-transform duration-500 hover:scale-[1.005] transform-gpu">
              {/* Internal Glows */}
              <div className="absolute top-0 right-0 w-[200px] md:w-[300px] h-[200px] md:h-[300px] bg-indigo-600/30 rounded-full blur-[60px] md:blur-[80px] -translate-y-1/2 translate-x-1/2 transform-gpu" />
              <div className="absolute bottom-0 left-0 w-[150px] md:w-[200px] h-[150px] md:h-[200px] bg-emerald-500/20 rounded-full blur-[40px] md:blur-[60px] translate-y-1/2 -translate-x-1/2 transform-gpu" />

              <div className="relative z-10">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/10 backdrop-blur-md text-[10px] md:text-xs font-bold text-indigo-300 mb-6">
                  <Sparkles size={12} />
                ...
                <p className="text-zinc-400 text-sm md:text-lg max-w-md mb-8 leading-relaxed">
                ...
                <div className="flex flex-col sm:flex-row items-stretch gap-4">
                  <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-1.5 pl-5 flex items-center justify-between backdrop-blur-md">
                    ...
                    <button 
                      onClick={handleCopy}
                      className="h-10 w-10 md:h-12 md:w-12 rounded-xl bg-indigo-600 hover:bg-indigo-500 flex items-center justify-center transition-all shadow-lg shadow-indigo-500/20 active:scale-90"
                    >
          ...
          >
             <GlassCard className="h-full p-5 md:p-6 flex flex-col justify-between group transform-gpu">
                <div>
          ...
          >
             <GlassCard className="h-full p-5 sm:p-8 transform-gpu">
                ...
                <div className="space-y-4">
                   <div className={`relative group p-4 rounded-2xl border transition-all duration-300 ${referralCount >= 5 ? 'bg-gradient-to-r from-emerald-50 to-white border-emerald-100' : 'bg-zinc-50 border-zinc-100 opacity-80'}`}>
                      <div className="flex items-center gap-4 relative z-10">
                          <div className={`h-10 w-10 md:h-12 md:w-12 shrink-0 rounded-full flex items-center justify-center border-4 text-base md:text-lg font-bold transition-all ${referralCount >= 5 ? 'bg-emerald-500 border-emerald-200 text-white shadow-lg shadow-emerald-200' : 'bg-white border-zinc-200 text-zinc-300'}`}>
                             {referralCount >= 5 ? <Check size={18} strokeWidth={3} /> : "1"}
                          </div>
                          <div className="flex-1">
                             <h4 className="font-bold text-sm md:text-base text-zinc-900">5 Verified Salons</h4>
                             <p className="text-[10px] md:text-xs text-zinc-500 font-medium mt-0.5">₹300 Cash Bonus</p>
                          </div>
                          {referralCount >= 5 ? (
                             <span className="px-2 md:px-3 py-1 rounded-full bg-white border border-emerald-100 text-emerald-600 text-[10px] md:text-xs font-bold shadow-sm">Paid</span>
                          ) : (
                   ...
                   <div className={`relative group p-4 rounded-2xl border transition-all duration-300 ${referralCount >= 10 ? 'bg-gradient-to-r from-indigo-50 to-white border-indigo-100' : 'bg-white border-zinc-100'}`}>
                      <div className="flex items-center gap-4 relative z-10">
                          <div className={`h-10 w-10 md:h-12 md:w-12 shrink-0 rounded-full flex items-center justify-center border-4 text-base md:text-lg font-bold transition-all ${referralCount >= 10 ? 'bg-indigo-600 border-indigo-200 text-white shadow-lg shadow-indigo-200' : 'bg-white border-zinc-200 text-zinc-300'}`}>
                             {referralCount >= 10 ? <Gift size={18} /> : "2"}
                          </div>
                          <div className="flex-1">
                             <h4 className="font-bold text-sm md:text-base text-zinc-900">10 Verified Salons</h4>
                             <p className="text-[10px] md:text-xs text-zinc-500 font-medium mt-0.5">₹800 Cash + Premium Welcome Kit</p>
                          </div>
                          {referralCount >= 10 ? (
                             <span className="px-2 md:px-3 py-1 rounded-full bg-indigo-600 text-white text-[10px] md:text-xs font-bold shadow-md shadow-indigo-200">Claim</span>
                          ) : (
          ...
          >
             <div className="h-full relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#FFEEEE] via-[#FFF5F5] to-white border border-pink-100 p-6 md:p-8 flex flex-col justify-between group transform-gpu">
                <div className="relative z-10">
                  ...
                  <p className="text-xs md:text-sm text-zinc-500 mt-2 font-medium">T-Shirt • Apron • Window Stickers</p>
                </div>
                
                {/* Image Section */}
                <div className="relative h-32 md:h-48 w-full flex items-center justify-center mt-4">
                  ...
                  <img 
                    ...
                    alt="Kit" 
                    className="relative z-10 w-32 md:w-48 drop-shadow-2xl transition-transform duration-700 ease-in-out group-hover:-translate-y-4 group-hover:scale-105 group-hover:rotate-3"
                    onError={(e) => {
          ...
          >
            <GlassCard className="p-6 md:p-8 transform-gpu">
                <h3 className="text-lg font-bold text-zinc-900 mb-6 flex items-center gap-2">
                  <ShieldCheck className="text-indigo-500" size={20} />
                  Verification Lifecycle
                </h3>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 relative mb-8">
                  ...
                  {[
                    ...
                    <div key={i} className="flex md:flex-col items-center gap-4 md:text-center group">
                       <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-white border-2 border-zinc-100 flex items-center justify-center font-black text-base md:text-lg text-zinc-300 shadow-sm group-hover:border-indigo-500 group-hover:text-indigo-600 group-hover:scale-110 transition-all duration-300">
                          {step.icon}
                       </div>
                       <div>
                ...
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center gap-5">
                  <div className="h-10 w-10 md:h-12 md:w-12 rounded-full bg-white flex items-center justify-center text-emerald-600 shadow-sm shrink-0">
                      <MessageCircle size={20} />
                  </div>
                  <div className="flex-1">
                      <h4 className="font-bold text-zinc-900 text-base md:text-lg flex items-center gap-2">
                        Shipping & Delivery
