...
import {
  ...
  Calendar, MapPin, Menu, X, Tag, Gift, Trash2, Loader2, CheckSquare, Square
} from "lucide-react";
...
// --- HELPER: GET LAST 14 DAYS DATES ---
const getLast14Days = () => {
    const dates = [];
    for (let i = 13; i >= 0; i--) { // 13 days ago se lekar aaj tak (0)
        const d = new Date();
...
export const AdminLogin = ({ onBack, onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if(!creds.username || !creds.password) { alert("Please fill all fields"); return; }
    setLoading(true);
    try {
        const { data } = await api.post("/admin/login", creds);
        if(data.success) { onLogin(); } else { alert("Invalid Access Credentials"); }
    } catch (error) {
      ...
      <div className="relative z-10 w-full max-w-md bg-zinc-900/50 backdrop-blur-xl border border-zinc-800 p-6 md:p-8 rounded-3xl shadow-2xl animate-fade-in-up">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-black/40 border border-zinc-800 rounded-xl p-1 flex items-center focus-within:border-zinc-600 transition-colors">
            ...
            <input type="text" placeholder="Username" className="bg-transparent w-full p-3 text-white outline-none placeholder:text-zinc-600" value={creds.username} onChange={(e) => setCreds({...creds, username: e.target.value})} />
          </div>
          <div className="bg-black/40 border border-zinc-800 rounded-xl p-1 flex items-center focus-within:border-zinc-600 transition-colors">
            <Lock className="text-zinc-500 ml-3 flex-shrink-0" size={18} />
            <input type="password" placeholder="Password" className="bg-transparent w-full p-3 text-white outline-none placeholder:text-zinc-600" value={creds.password} onChange={(e) => setCreds({...creds, password: e.target.value})} />
          </div>
          <button disabled={loading} className="w-full bg-white text-black font-bold py-4 rounded-xl hover:bg-zinc-200 transition mt-4 shadow-lg hover:shadow-white/10 active:scale-95 transform duration-150 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
            {loading ? <Loader2 className="animate-spin" size={20} /> : "Enter God Mode"}
          </button>
        </form>
        <button onClick={onBack} className="w-full text-zinc-500 text-xs mt-6 hover:text-white transition">Return to Platform</button>
      </div>
    </div>
  );
};
                        ...
                        <Marker key={salon._id} position={[salon.latitude, salon.longitude]}>
                            <Popup>
                                <div className="text-zinc-900 font-sans p-1 min-w-[150px]">
                                    ...
                                    <div className={`mt-1 text-[10px] font-bold px-2 py-0.5 rounded-full inline-block ${salon.verified ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{salon.verified ? "Verified" : "Pending"}</div>
                                </div>
                            </Popup>
                        </Marker>
                    )
...
export const AdminDashboard = ({ salons = [], setSalons, onLogout }) => {
  ...
  // 14 Days Calculation for Header
  ...
  useEffect(() => {
    ...
    socket.on("admin_stats_update", () => { console.log("⚡ Admin Stats Updated via Socket"); fetchDashboardData(); });
    return () => socket.disconnect();
  }, []);

  // --- INITIAL FETCH ---
  useEffect(() => { fetchDashboardData(); fetchSalons(); }, []);
  ...
  const fetchDashboardData = async () => {
    try {
        ...
        if(data.success) { setStats(data.stats); setActivityLog(data.activity); }
  ...
  const fetchSalons = async () => {
    ...
    } catch (error) { console.error("Salons Fetch Error", error); }
  };
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (data.success) {
        // Sort by 'lastActiveAt' to bring active users to top
  ...
  const toggleVerify = async (id, currentStatus) => {
    try {
        ...
        if(data.success) { setLocalSalons(prev => prev.map(s => s._id === id ? { ...s, verified: !currentStatus } : s)); }
  ...
  const deleteSalon = async (id) => {
    if(window.confirm("CONFIRM DELETION: This action cannot be undone.")) {
      try {
        ...
        if(data.success) { setLocalSalons(prev => prev.filter(s => s._id !== id)); alert(data.message); }
      } catch (error) { alert("Delete Failed"); }
    }
  };

  const deleteUser = async (id) => {
    if(window.confirm("⚠️ DANGER: Permanently delete this user?")) {
        try {
        ...
        } catch (error) { console.error("Delete User Error", error); alert("Failed to delete user."); }
    }
  };
  ...
  const navItems = [
    ...
    { id: "users", icon: Users, label: "Play Store Testing" }, // Renamed for clarity
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-indigo-500 selection:text-white flex flex-col md:flex-row overflow-hidden">
      ...
      <div className="md:hidden h-16 bg-zinc-900/80 backdrop-blur-xl border-b border-zinc-800 flex items-center justify-between px-4 sticky top-0 z-50">
          ...
          <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 text-zinc-400 hover:text-white"><Menu size={24} /></button>
      </div>
        ...
        <div className="fixed inset-0 z-[100] md:hidden">
            ...
            <div className="absolute top-0 left-0 w-3/4 max-w-[280px] h-full bg-zinc-900 border-r border-zinc-800 flex flex-col shadow-2xl animate-[slideRight_0.3s_ease-out]">
                ...
                <div className="p-4 space-y-2">
                    {navItems.map((item) => (
                        <button key={item.id} onClick={() => { setActiveTab(item.id); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${activeTab === item.id ? "bg-indigo-600/10 text-indigo-400 border border-indigo-600/20" : "text-zinc-400 hover:text-white hover:bg-white/5"}`}>
                          <item.icon size={18} /> {item.label}
                        </button>
                    ))}
                </div>
                <div className="mt-auto p-4 border-t border-zinc-800">
                    <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-red-500/10 text-red-400 border border-red-500/20 text-sm font-bold hover:bg-red-500/20 transition"><LogOut size={16} /> Logout</button>
                </div>
            </div>
        </div>
      )}
      ...
      <aside className="hidden md:flex w-64 border-r border-zinc-800 bg-zinc-900/30 flex-col backdrop-blur-xl z-20 h-screen sticky top-0">
        ...
        <div className="p-4 space-y-1">
          ...
          {navItems.map((item) => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${activeTab === item.id ? "bg-indigo-600/10 text-indigo-400 border border-indigo-600/20" : "text-zinc-400 hover:text-white hover:bg-white/5"}`}>
              <item.icon size={18} /> {item.label}
            </button>
          ))}
        </div>
        <div className="mt-auto p-4 border-t border-zinc-800">
          <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-red-500/10 text-red-400 border border-red-500/20 text-sm font-bold hover:bg-red-500/20 transition"><LogOut size={16} /> Exit Founder Mode</button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-[calc(100vh-64px)] md:h-screen overflow-hidden relative">
        ...
        <header className="hidden md:flex h-16 border-b border-zinc-800 items-center justify-between px-8 bg-zinc-900/30 backdrop-blur-md sticky top-0 z-10">
          <h2 className="text-xl font-bold flex items-center gap-2">
            {activeTab === 'overview' ? "Command Center" : activeTab === 'users' ? "14-Day Testing Tracker" : activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-bold"><Globe2 size={14} /> Global: Jodhpur</div>
            <div className="w-8 h-8 bg-zinc-800 rounded-full flex items-center justify-center border border-zinc-700 text-zinc-400 hover:text-white cursor-pointer transition"><Bell size={16} /></div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 scrollbar-hide">
                ...
                {[
                  ...
                  <div key={i} className="bg-zinc-900/50 border border-zinc-800 p-5 md:p-6 rounded-2xl relative overflow-hidden group hover:border-zinc-700 transition-all">
                    <div className="flex justify-between items-start mb-4">
                      <div className={`p-3 rounded-xl bg-zinc-800/50 ${stat.color} group-hover:scale-110 transition-transform duration-300`}><stat.icon size={22} /></div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-zinc-900/50 border border-zinc-800 rounded-3xl p-6 md:p-8 relative overflow-hidden">
                  <div className="flex justify-between items-center mb-8">
                    <div><h3 className="font-bold text-lg text-white">Projected Growth</h3><p className="text-zinc-500 text-xs">Visual Representation of Scale</p></div>
                  ...
                  <div className="h-48 md:h-64 flex items-end justify-between gap-1 md:gap-2">
                    {[35, 45, 30, 50, 45, 60, 55, 70, 65, 80, 75, 90, 85, 100].map((h, i) => (
                      <div key={i} className="w-full bg-zinc-800/50 rounded-t-lg relative group hover:bg-indigo-600/50 transition-all duration-300" style={{ height: `${h}%` }}></div>
                    ))}
                  </div>
                </div>
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-6 flex flex-col h-[350px] md:h-[400px]">
                  <h3 className="font-bold text-lg text-white mb-6 flex items-center gap-2"><Zap size={18} className="text-yellow-400" /> Recent Activity</h3>
                  <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                    {activityLog.length === 0 ? <p className="text-zinc-500 text-sm text-center mt-10">No recent activity found.</p> : activityLog.map((log, i) => (
                        <div key={i} className="flex gap-3 relative pl-4 border-l border-zinc-800">
                            ...
                            <div>
                            <p className="text-sm text-zinc-300 leading-snug"><span className="font-bold text-white">{log.userId?.name || "User"}</span> joined queue at <span className="text-zinc-100 font-medium">{log.salonId?.salonName || "Salon"}</span></p>
          ...
          {activeTab === "salons" && (
            <div className="animate-[slideUp_0.4s_ease-out]">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div><h2 className="text-2xl font-black text-white">Partner Management</h2><p className="text-zinc-500 text-sm">Geospatial view of all registered partners.</p></div>
                <div className="relative w-full md:w-auto">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500" size={16}/>
                  <input type="text" placeholder="Search salons..." className="bg-zinc-900 border border-zinc-800 rounded-xl pl-10 pr-4 py-3 text-sm text-white focus:border-indigo-500 outline-none w-full md:w-64 transition-all" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                </div>
              </div>
                    ...
                    <tbody className="divide-y divide-zinc-800/50">
                        ...
                        <tr key={salon._id} className="hover:bg-white/[0.02] transition">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-lg bg-zinc-800 flex items-center justify-center font-bold text-white text-xs border border-zinc-700 flex-shrink-0">{salon.salonName.substring(0, 2).toUpperCase()}</div>
                                <div><p className="font-bold text-white text-sm">{salon.salonName}</p><p className="text-xs text-zinc-500">{salon.address}</p></div>
                            </div>
                            </td>
                            <td className="px-6 py-4">
                            {salon.verified ? <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-green-500/10 text-green-400 border border-green-500/20 text-xs font-bold"><CheckCircle size={12}/> Verified</span> : <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 text-xs font-bold"><AlertTriangle size={12}/> Pending</span>}
                            </td>
                            <td className="px-6 py-4 text-white font-mono">₹{salon.revenue?.toLocaleString() ?? 0}</td>
                            <td className="px-6 py-4 text-right">
                            <button onClick={() => toggleVerify(salon._id, salon.verified)} className="text-indigo-400 hover:underline text-xs mr-3">{salon.verified ? "Revoke" : "Verify"}</button>
                            <button onClick={() => deleteSalon(salon._id)} className="text-red-400 hover:underline text-xs">Ban</button>
                            </td>
                        </tr>
                        ))}
                    </tbody>
          ...
          {/* ---------------- USERS VIEW (PLAY STORE TRACKING) ---------------- */}
          {activeTab === "users" && (
             <div className="animate-[slideUp_0.4s_ease-out]">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h2 className="text-2xl font-black text-white">Play Store Testing Tracker</h2>
                    <p className="text-zinc-500 text-sm">Monitor 20 testers for 14 continuous days.</p>
                  </div>
                  <div className="flex gap-2">
                      <span className="px-3 py-1 bg-zinc-800 rounded-full text-xs text-zinc-400 border border-zinc-700 whitespace-nowrap">
                        Total Testers: {userList.length}
                      </span>
                  </div>
                </div>

                {loading ? (
                   <div className="flex justify-center py-20"><div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-indigo-500"></div></div>
                ) : (
                  <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl overflow-hidden">
                    <div className="overflow-x-auto">
                        ...
                        <thead className="bg-zinc-900 text-zinc-500 font-bold uppercase text-[10px] tracking-wider">
                            ...
                            {/* 🔥 14 DAYS HEADER VISUALIZATION 🔥 */}
                            <th className="px-6 py-4">
                                <div className="flex items-center gap-1">
                                    <span className="mr-2">14-Day Streak</span>
                                    <span className="text-[9px] font-normal normal-case opacity-50">(Oldest → Today)</span>
                                </div>
                            </th>
                            <th className="px-6 py-4 text-right">Actions</th>
                            </tr>
                        ...
                        <tbody className="divide-y divide-zinc-800/50">
                            {userList.length === 0 ? (
                               <tr><td colSpan="4" className="text-center py-10 text-zinc-600">No users found.</td></tr>
                            ) : (
                              userList.map((user) => {
                                // Calculate Streak Count
                                const streakCount = user.activeDates ? user.activeDates.length : 0;
                                const isActiveToday = user.activeDates?.includes(new Date().toISOString().split('T')[0]);
                                ...
                                <tr key={user._id} className={`hover:bg-white/[0.02] transition`}>
                                  ...
                                  <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs flex-shrink-0 bg-gradient-to-br from-indigo-500 to-purple-600`}>
                                  ...
                                  {/* 🔥 14-DAY CALENDAR VISUALIZATION 🔥 */}
                                  <td className="px-6 py-4">
                                    <div className="flex flex-col gap-2">
                                        <div className="flex items-center gap-1">
                                            {last14Days.map((date, index) => {
                                                const isActive = user.activeDates?.includes(date);
                                                return (
                                                    <div key={index} className="group relative">
                                                        ...
                                                        {/* Tooltip on Hover */}
                                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-zinc-800 text-white text-[10px] rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap border border-zinc-700 z-10 transition-opacity">
                                                            {date} {isActive ? "✅" : "❌"}
                                                        </div>
                                                    </div>
                                                );
                                        ...
                                        <div className="text-[10px] text-zinc-500 font-medium">
                                            Total Active Days: <span className="text-white font-bold">{streakCount} / 14</span>
                                        </div>
                                    </div>
                                  </td>
                                  ...
                                  <td className="px-6 py-4 text-right">
                                    <div className="flex items-center justify-end gap-3">
                                        {/* Call Button if not active today */}
                                        {!isActiveToday && (
                                            <a href={`tel:${user.phone}`} className="p-2 bg-yellow-500/10 text-yellow-500 rounded-lg hover:bg-yellow-500/20 border border-yellow-500/20 transition" title="Call User to Open App">
                                                <Phone size={14} />
                                            </a>
                                        )}
                                        <button onClick={() => deleteUser(user._id)} className="p-2 bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500/20 transition border border-red-500/20" title="Delete User">
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                  </td>
